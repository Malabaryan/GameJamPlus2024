...
namespace BNG {
    public class SnapZone : MonoBehaviour {
        ...
        void Update() {
            ...
            if (HeldItem != null) {
                ...
                if (HeldItem.BeingHeld || (HeldItem.transform.parent != transform && ParentObjectToSnapZone)) {
                    ...
                    // We can use update since the rigidbody is not in effect
                    HeldItem.transform.position = transform.position;
        ...
        void FixedUpdate() {
            // Move object manually if we didn't parent it
            // We can use potentially? use update since the rigidbody is not in effect
            if (heldItemRigid != null) {
                heldItemRigid.MovePosition(transform.position);
                heldItemRigid.MoveRotation(transform.rotation);
            }
        }
